    ...
    <div class="h-fit pb-20 m-auto mt-[20vh] w-3/4 bg-gray-100">
      ...
      <div class="flex flex-col">
        <div
          ...
          :key="project.id"
          class="flex flex-row w-[90%] md:w-[80%] my-4 bg-gray-300 m-auto justify-between"
        >
          <p
            class="px-6 py-4 text-base md:text-lg font-bold text-teal-950 text-md justify-center max-w-[144px] md:max-w-[1000px] break-all"
          >
            {{ project.title }}
          </p>
          <div class="pr-6 flex items-center">
            <button
              class="btn pointer-events-auto mr-2 md:mr-0 md:ml-4 bg-teal-500 btn-icon btn-icon-sm text-white w-[35px] h-[35px] rounded-full"
              @click="viewTodos(project.id)"
            >
              <font-awesome-icon icon="fa-solid fa-folder-open" />
            </button>
            <button
              class="btn pointer-events-auto mr-2 md:mr-0 md:ml-4 bg-teal-500 btn-icon btn-icon-sm text-white w-[35px] h-[35px] rounded-full"
              @click="onEdit(project, 'edit')"
            >
        ...
        <EmptyState
          ...
          emptyText="Let's organize! Create your first project and conquer those tasks"
        />
      </div>
    </div>
    <ProjectModal
      v-if="showPopup"
      :projectDetails="editData"
      @exit="handleClosePopup"
      @onSave="onSave"
    />
  </main>
</template>
