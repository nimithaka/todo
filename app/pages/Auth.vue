    ...
    <section class="bg-gray-50 dark:bg-gray-900">
      <div class="flex justify-center items-center mt-52">
        <div class="w-full max-w-md">
          <form id="authenticationForm" class="bg-white border border-gray-100 shadow-sm rounded px-8 pt-6 pb-8">
            <h1 class="text-xl flex items-center justify-center font-bold leading-tight tracking-tight text-teal-950 md:text-2xl dark:text-white">{{getTitleText}}</h1>
            ...
            <div class="password-input">
              ...
              <input
                ...
                @input="validateInputField($event)"
              />
              <span v-if="!errMsg" class="flex text-xs text-red-700 h-5" id="pwdErr"></span>
              <span v-else class="flex text-xs text-red-700 h-5 justify-center items-center">{{errMsg}}</span>
            </div>
              <button
              ...
              <p v-if="isLogin" class="text-sm font-light text-gray-500 dark:text-gray-400">Don’t have an account yet? <a href="#" class="font-medium text-teal-500 hover:text-teal-800" @click="toggleAuthForSignup">Sign up</a></p>
              <p v-else class="text-sm font-light text-gray-500 dark:text-gray-400">Already have an acoount? <a href="#" class="font-medium text-teal-500 hover:text-teal-800" @click="toggleAuthForSignin">Sign in</a></p>
          </form>
        </div>
      </div>
      <Toast v-if="showSuccessToast" message="Account created successfully!" :isSuccess="true"></Toast>
    </section>
  </template>
        ...
        async authenticate() {
          ...
          if(isValidationPassed) {
              ...
              if(this.isLogin) {
                ...
                const body = JSON.stringify({email: this.email, pwd: this.password})
                try {
