          ...
          <div class="flex flex-col mt-4 items-center justify-center">
            ...
            <form class="flex flex-col mt-2 w-full" id="projectForm">
              <div>
                ...
                <span
                  v-if="errMsg"
                  class="flex text-xs text-red-700 h-5 mt-1 justify-center items-center"
                  id="ptError"
                  >{{ errMsg }}</span
                >
                <span
                  v-else
                  class="flex text-xs text-red-700 h-5 mt-1"
                  id="ptError"
                ></span>
              </div>
              <button
                type="button"
                class="bg-teal-500 hover:bg-teal-800 text-white font-bold py-2 px-4 rounded focus:outline-none"
                @click="onSave"
              >
...
export default {
  ...
  computed: {
    getTitle() {
      return this.projectDetails?.method === "edit"
        ? "Edit Project Title"
        : "Add New Project";
    },
  },
